...
describe('AppController (e2e)', () => {
  ...
  describe(`/?index=X (GET)`, () => {
    ...
    it(`should return a 400 if an non-number index is passed in`, () => {
      return request(app.getHttpServer()).get(`/?index=not-a-number`).set(`x-api-key`, `SECRET`).expect(400)
    })
    it(`should return a 400 if an negative index is passed in`, () => {
      return request(app.getHttpServer()).get(`/?index=-1`).set(`x-api-key`, `SECRET`).expect(400)
    })
    it(`should return a 400 if an index greater than 14 is passed in`, () => {
      return request(app.getHttpServer()).get(`/?index=15`).set(`x-api-key`, `SECRET`).expect(400)
    })
  })
})
